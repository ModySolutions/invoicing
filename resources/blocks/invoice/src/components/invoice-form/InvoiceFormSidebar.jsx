...
const InvoiceSettings = () => {
	...
	}, [ invoiceTaxAmount, invoiceDiscountAmount, setInvoice ] );
	...
	return (
		<div className="flex flex-column gap-1">
			<div className="flex flex-column">
				...
				>
					{ settings?.spain_iva &&
						Object.entries( settings?.spain_iva ).map(
							( [ value, label ] ) => (
								<option
			...
			<div className="flex flex-column">
				<label htmlFor="invoice_discount_amount">
					{ __( 'I.R.P.F.', 'app' ) }
				</label>
				<select
				...
				>
					{ settings?.spain_irpf &&
						Object.entries( settings?.spain_irpf ).map(
							( [ value, label ] ) => (
								<option
