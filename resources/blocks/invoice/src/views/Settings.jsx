...
import DateFormats from '../../../../scripts/tools/DateFormats';
import { useSettings } from '../contexts/SettingsContext';

const Settings = () => {
	const { settings, setSettings, loading, error } = useSettings();
	const [ formData, setFormData ] = useState( settings );
	const [ dateFormats ] = useState( DateFormats );
	const [ updating, setUpdating ] = useState( false );
	const [ selectedDateFormat, setSelectedDateFormat ] = useState(
		formData?.invoice_date_format ?? 'F j, Y'
	);
	const [ lastInvoiceNumber, setLastInvoiceNumber ] = useState(
		formData?.invoice_last_number ?? 0
	);
	...
	useEffect( () => {
		if ( null !== settings ) {
			...
			setLastInvoiceNumber(
				formData?.invoice_last_number ?? settings?.invoice_last_number
			);
			setSelectedDateFormat(
				formData?.invoice_date_format ?? settings?.invoice_date_format
			);
		}
	}, [ formData, settings ] );
			...
			.then( ( response ) => {
				...
				if ( success ) {
					...
					toast.error(
						message ||
							__(
								'There was an error updating invoice settings.'
							),
						{
							autoClose: 10000,
						}
			...
			{ formData && (
				<form className={ 'container' } onSubmit={ handleSubmit }>
					...
					<div className="form-group col-6">
						<label htmlFor="invoice_last_number">
						...
						<em className="text-gray-60 text-sm">
							{ __( 'The last invoice number you have used.' ) }{ ' ' }
							<br />
							{ __(
								'We automatically will keep your next invoice number updated.'
							) }
						</em>
					</div>
					<div className="form-group col-6">
						<label htmlFor="invoice_date_format">
							{ __( 'Date format' ) }
						</label>
						<select
							className="input-lg"
					...
					<div className="form-group col-6">
						<label htmlFor="invoice_currrency">
							{ __( 'Currency' ) }
						</label>
						<select
							className="input-lg"
