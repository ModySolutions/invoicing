...
const Business = () => {
	const { settings, setSettings, europeCountries, loading, error } =
		useSettings();
	...
	useEffect( () => {
		if ( ! formData && null !== settings ) {
			...
			setSelectedCountryCode(
				selectedCountryCode ??
					settings?.invoice_business_fni_country_code
			);
			setSelectedCountry(
				selectedCountry ?? settings?.invoice_business_country
			);
			setSelectedState(
				selectedState ?? settings?.invoice_business_state
			);
		}
		...
		apiFetch( {
			...
			.then( ( response ) => {
				...
				if ( success ) {
					...
					toast.error(
						message ||
							__( 'There was an error updating business data.' ),
						{
							autoClose: 10000,
						}
					);
